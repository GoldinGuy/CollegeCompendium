					...
					{loading ? (
						...
						<div className="flex flex-col">
							{TEXTBOOK_TAGS.map((category, i) => {
								return (
									<div id={category} className="px-10 pt-6" key={category}>
										<span className="capitalize text-md"> {category}</span>
										<div className="flex flex-col flex-wrap" key={category + i}>
						...
						<div className="flex flex-col">
							{TEXTBOOK_TAGS.map((category, i) => {
								return (
									<div
										id={category}
										className="pt-6 px-4 sm:px-10"
										key={category}
									>
										<span className="capitalize text-md mb-4"> {category}</span>
										<div
											className="flex flex-row flex-wrap justify-center sm:justify-start"
											key={category + i}
										>
											{textbooks.map((text, j) => {
												if (text.category.includes(category.toLowerCase())) {
													return (
														<a
															href={text.url}
